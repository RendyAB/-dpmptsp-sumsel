...
@section('titleadminprovinsi', 'View Data Investasi Admin Provinsi - DPMPTSP Sumatera Selatan')
@section('konten_admin_provinsi')
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-2">
            <form method="GET" action="{{ route('investasi_tampil_provinsi', $kab_kota_id) }}" class="flex flex-wrap gap-2">
                <select
                    onchange="window.location='{{ url('/admin-provinsi/investasi/view') }}/'+this.value+'?triwulan={{ $triwulan }}&tahun={{ $tahun }}'"
                    class="border p-2 rounded">
                    <option value="" disabled selected hidden>-- Pilih Kab/Kota --</option>
                    @foreach ($kabkotaList as $kota)
                        <option value="{{ $kota->id }}" {{ $kab_kota_id == $kota->id ? 'selected' : '' }}>
                            {{ $kota->nama }}
                        </option>
                    @endforeach
                </select>
                ...
                <input type="number" name="tahun" value="{{ $tahun }}" class="border p-2 rounded w-24">
            ...
            @if ($kab_kota_id)
                <div class="flex flex-row gap-2">
                    <a href="{{ route('investasi_export_excel_provinsi', ['triwulan' => $triwulan, 'tahun' => $tahun, 'kab_kota' => $kab_kota_id]) }}"
            ...
            @if ($kab_kota_id)
                <a href="{{ route('export_rekap_investasi_tahunan_provinsi', ['tahun' => $tahun, 'kab_kota' => $kab_kota_id]) }}"
                    class="exportBtn bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-center">
    ...
    <h2 class="text-xl font-bold mb-4">
        Data Investasi – {{ $nama_kab_kota ?? 'Pilih Kab/Kota' }}
        @if ($nama_kab_kota)
            – Triwulan {{ $triwulan }} Tahun {{ $tahun }}
        @endif
    </h2>

    @if ($nama_kab_kota && $kategori->count() > 0)
            ...
            <table class="table-auto border-collapse w-full border border-gray-300 text-sm">
                ...
                <tbody>
                    ...
                    @foreach ($kategori as $kat)
                        <tr class="font-bold">
                            <td colspan="14" class="border px-2 py-1">{{ strtoupper($kat->nama) }}</td>
                        </tr>
                        ...
                        @foreach ($kat->sektorInvestasi as $sektor)
                            ...
                            <tr>
                                ...
                                <td class="border px-2 py-1 text-right">
                                    {{ number_format($pmdnRealisasi, 2, ',', '.') }}</td>
                        ...
                        <tr class="bg-gray-200 font-bold">
                            <td colspan="2" class="text-right border px-2 py-1">JUMLAH {{ strtoupper($kat->nama) }}
                            </td>
                            <td class="border px-2 py-1 text-center">{{ $totalPmaLkpm }}</td>
                            <td class="border px-2 py-1 text-right">
                                {{ number_format($totalPmaRealisasi, 2, ',', '.') }}</td>
                            ...
                            <td class="border px-2 py-1 text-right">
                                {{ number_format($totalPmdnRealisasi, 2, ',', '.') }}</td>
                    ...
                    <tr class="bg-green-200 font-bold">
                        ...
                        <td class="border px-2 py-2 text-right">{{ number_format($grandPmaRealisasi, 2, ',', '.') }}
                        </td>
                        ...
                        <td class="border px-2 py-2 text-right">{{ number_format($grandPmdnRealisasi, 2, ',', '.') }}
                        </td>
    ...
    @else
        <p class="text-gray-500 mt-4">Silakan Pilih Kabupaten/Kota Untuk Melihat Data Investasi</p>
    @endif
    ...
    @if ($nama_kab_kota && $kategori->count() > 0)
        <div class="mt-14">
            <div class="flex items-center justify-between mb-4">
                <form method="GET" action="{{ route('investasi_tampil_provinsi', $kab_kota_id) }}"
                    class="flex items-center justify-between mb-8">
                    <div class="flex gap-2">
                        <select name="kategori" class="border p-2 rounded">
                            <option value="">-- Semua Sektor --</option>
                            <option value="Sektor Primer" {{ request('kategori') == 'Sektor Primer' ? 'selected' : '' }}>
                                Primer
                            </option>
                ...
                <button id="exportDiagramInvestasiProvinsi"
    ...
    <script>
        ...
        document.getElementById('exportDiagramInvestasiProvinsi').addEventListener('click', function() {
            overlay.classList.remove('hidden');
            const {
                jsPDF
            } = window.jspdf;
