<table>
    <thead>
        ...
        <tr>
            ...
            <th rowspan="2">REALISASI PMA + PMDN TW.{{ $triwulan }} TH.{{ $tahun }}</th>
    ...
    <tbody>
        ...
        @foreach ($kategori as $kat)
            ...
            @foreach ($kat->sektorInvestasi as $sektor)
                @php
                    $inv = $sektor->investasi->first();
                    $pmaLkpm = $inv->lkpm_pma ?? 0;
                    $pmdnLkpm = $inv->lkpm_pmdn ?? 0;
                    $pmaRealisasi = $inv->realisasi_pma ?? 0;
                    $pmdnRealisasi = $inv->realisasi_pmdn ?? 0;
                    $pmaTki = $inv->tki_pma ?? 0;
                    $pmdnTki = $inv->tki_pmdn ?? 0;
                    $pmaTka = $inv->tka_pma ?? 0;
                    $pmdnTka = $inv->tka_pmdn ?? 0;
